...
**用途**：KIT TA 数据的**交互式可视化工具**。加载二维矩阵（波长 × 时间）的 `.csv` 或 `.xlsx`，支持按时间切片出光谱，或按波长切片出动力学曲线。 **核心能力**： - 上下两个同步面板：固定时间光谱与固定波长 kinetics 可同时显示。 - 时间 / 波长方向可选局部平均：时间列 ±1 或 ±2；波长窗口 2 nm / 3 nm。 - 按波长自动 **Baseline Correction** 一键切换：程序从 pre-pump 区域估计每个波长自己的 baseline，优先使用自动识别的信号 onset / time-zero 前数据；若无法识别则退回到负时间点，再不行用最早几列。再次点击按钮会恢复原始未修正数据；已修正时图像区域会出现亮橙色 `BASELINE CORRECTED` 提示。 - 多曲线叠加、曲线选择/删除、鼠标悬停最近点读数、Matplotlib 工具栏缩放、独立坐标轴范围、Flip Y、JSON 设置保存/读取、Windows 高 DPI 自适应。 **使用方法**：运行后 Browse / Load 选择 `.csv` 或 `.xlsx`，设置时间/波长参数后点击 **Add Spectrum** 或 **Add Kinetics**，需要时点击 **Baseline Correction**。 **输出**：通过 **Save PNG + XLSX** 保存当前绘图与曲线数据。
