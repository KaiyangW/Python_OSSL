...
from fit_multistart import run_multistart_recon
...
# --- Dynamic Reconvolution Math Engine ---
...
def auto_format_time(tau_ns):
    ...
    try:
        ...
        return tau_ns # 如果不是数字，直接返回原值

def stretched_exp_reconv(params, t_uniform, irf_uniform, dt, num_exp):
    shift = params[0]
    bkg = params[1]
    scatter_amp = params[2] 
    
    irf_shifted = np.interp(t_uniform, t_uniform - shift, irf_uniform, left=0, right=0)
    
    t_decay = np.arange(len(t_uniform)) * dt
    ...
    convoluted = fftconvolve(irf_shifted, decay, mode='full')[:len(t_uniform)] * dt
    
    model_total = convoluted + (irf_shifted * scatter_amp) + bkg
    return model_total

def residuals(params, t_uniform, data_uniform, irf_uniform, dt, fit_mask, num_exp):
    model = stretched_exp_reconv(params, t_uniform, irf_uniform, dt, num_exp)
...
def run_fitting_process(app_state, xmin, xmax, initial_taus, fixed_t_flags, initial_betas, fixed_b_flags, num_exp, phos_idx, calc_pf_df, avg_comps_str, plqy_val=0.0, pf_comp='C1', df_comp='C2', subtract_scatter=True, calc_risc=False):
    """
    Executes the optimization and updates the plots.
    Note: app_state, phos_idx, and calc_pf_df are passed from the main GUI script.
    """
    t = app_state['t']
    data = app_state['data']
    irf = app_state['irf']
    irf_display = app_state['irf_display']
    dt = app_state['dt']
    ax = app_state['ax']
    ...
    if not np.any(fit_mask): return
    
    print(f"\nRunning {num_exp}-component fit between {xmin:.2f} ns and {xmax:.2f} ns...")
    ...
    p0 = [0.0, bkg_guess, max_counts * 0.01] 
    lower_bounds = [-np.inf, 0, 0] 
    upper_bounds = [np.inf, max_counts, max_counts * 10] 
    ...
    model_full = stretched_exp_reconv(res.x, t, irf, dt, num_exp)
    ...
    for i in range(num_exp):
        ...
        # num_ave: 1st moment <t> = (tau/beta)*Gamma(1/beta), used to report single-component lifetime & compute absolute area
        num_ave = (tau_i / beta_i) * gamma(1.0 / beta_i)
        area_i = B_i * num_ave
        total_area += area_i

        # int_ave: 2nd moment / 1st moment = tau*Gamma(2/beta)/Gamma(1/beta), intensity-averaged lifetime for this component.
        int_ave = tau_i * (gamma(2.0 / beta_i) / gamma(1.0 / beta_i))

        components.append({
            'B': B_i, 'tau': tau_i, 'beta': beta_i,
            'num_ave': num_ave, 'int_ave': int_ave, 'area': area_i, 'is_phos': False
        })
        
    if total_area == 0: total_area = 1e-9
    ...
    attach_param_indices(
        components, num_exp,
        amp_idx_fn=lambda i: 3 + 3 * i,
        tau_idx_fn=lambda i: 4 + 3 * i,
        beta_idx_fn=lambda i: 5 + 3 * i,
    )
    ...
    if avg_comps_str:
        try:
            # Parse string like "1, 2" or "2,3,4" into 0-based indices [0, 1] or [1, 2, 3]
            ...
            if valid_indices:
                # Physics Note: We calculate the combined average lifetime for the SELECTED components.
                # We multiply each component's effective lifetime (int_ave) by its absolute area,
                # and then divide by the sum of areas of ONLY the selected components (sum_area).
                # This correctly acts as multiplying by their "relative area fraction" among the selection.
                sum_area_tau = sum([components[idx]['area'] * components[idx]['int_ave'] for idx in valid_indices])
    ...
    # ==========================================
    # --- Advanced Physics & Area Logic (Area Difference Method) ---
    # ==========================================
    # Delegate all PF/DF area calculations to the standalone engine.
    # The engine also applies the phosphorescence tag to the component dict.
    area_phos = 0.0
    area_pf = area_df = total_fluo_area = pf_ratio_clean = df_ratio_clean = phos_percent_total = 0.0
    total_data_area = scatter_area = None

    if calc_pf_df:
        area_result        = Area_Analysis_Engine.calc_recon_difference_areas(
                                 t, data, irf, xmin, xmax, bkg, shift, scatter_amp, components, phos_idx, 
                                 subtract_scatter=subtract_scatter)
        area_pf            = area_result['area_pf']
        area_df            = area_result['area_df']
        area_phos          = area_result['area_phos']
        total_fluo_area    = area_result['total_fluo_area']
        ...
        scatter_area       = area_result['scatter_area']
    else:
        # Still tag phosphorescence component even when PF/DF is not requested
        if phos_idx != -1 and phos_idx < len(components):
            components[phos_idx]['is_phos'] = True
            area_phos = components[phos_idx]['area']

    # ==========================================
    # --- PLOTTING AND UPDATING UI ---
    # ==========================================
    ...
    app_state['fit_line'], = ax.plot(t_plot, model_plot, color='magenta', linewidth=1.5, linestyle='solid', label='Fit')
    
    if app_state['res_line']:
        app_state['res_line'].remove()
        
    weighted_res = (data[fit_mask] - model_plot) / np.sqrt(np.maximum(data[fit_mask], 1))
    app_state['res_line'], = ax_res.plot(t_plot, weighted_res, color='limegreen', linewidth=1, alpha=0.8)
    ...
    SEP = "----------------------------------------------\n"
    text_str = (
        f"X2_R: {chi_sq:.3f} | Shift: {shift:.3f} ns\n"
        f"Bkg: {bkg:.1f} | Scatt: {scatter_amp:.1e}\n"
        + SEP
    )
    ...
    if calc_pf_df:
        text_str += SEP
        sub_tag = " -scatt" if subtract_scatter else ""
        text_str += f"[Area Diff (Recon){sub_tag}]\n"
    ...
    valid_irf_display = irf_display[fit_mask]
    
    if len(valid_y_data) > 0:
        y_max = max(np.max(valid_y_data), np.max(valid_irf_display))
        min_data_val = np.min(valid_y_data[valid_y_data > 0]) if np.any(valid_y_data > 0) else 1
        min_irf_val = np.min(valid_irf_display[valid_irf_display > 0]) if np.any(valid_irf_display > 0) else 1
        y_min = min(min_data_val, min_irf_val)
        ax.set_ylim(max(0.5, y_min * 0.5), y_max * 1.5)
        
    max_res = np.max(np.abs(weighted_res))
    y_lim_res = max(5, max_res * 1.2) 
    ax_res.set_ylim(-y_lim_res, y_lim_res)
    ...
    params_list = [
        ...
        {'Parameter': 'Shift (ns)', 'Value': shift},
        {'Parameter': 'Background', 'Value': bkg},
        {'Parameter': 'Scatter_Amp', 'Value': scatter_amp} 
    ]
    ...
    avg_rows = []
    if selected_avg_tau is not None:
        avg_rows.append({'Parameter': f'Avg_Int_Ave_Tau_C{selected_indices_display} (ns)', 'Value': selected_avg_tau})
        avg_rows.append({'Parameter': f'Avg_Int_Ave_Tau_C{selected_indices_display}_StdErr (ns)', 'Value': selected_avg_tau_stderr})
    if selected_mean_avg_tau is not None:
        avg_rows.append({'Parameter': f'Avg_Num_Ave_Tau_C{selected_indices_display} (ns)', 'Value': selected_mean_avg_tau})
        avg_rows.append({'Parameter': f'Avg_Num_Ave_Tau_C{selected_indices_display}_StdErr (ns)', 'Value': selected_mean_avg_tau_stderr})
    append_section(params_list, avg_rows)
    
    if calc_pf_df:
        append_section(params_list, [
            {'Parameter': '--- Area Diff Method (Recon) ---', 'Value': '---'},
            {'Parameter': 'Total_Raw_Data_Area (fit window)', 'Value': total_data_area},
            {'Parameter': 'Scatter_Area_Subtracted', 'Value': scatter_area},
            {'Parameter': 'PF_Area_Diff_Sub_Scatter (Recon)', 'Value': area_pf},
            {'Parameter': 'DF_Area_Analytical', 'Value': area_df},
            {'Parameter': 'Total_Fluorescence_Area', 'Value': total_fluo_area},
            {'Parameter': 'Phosphorescence_Area', 'Value': area_phos},
            {'Parameter': 'Phosphorescence_Percentage (%)', 'Value': phos_percent_total},
            {'Parameter': 'Clean_PF_Percentage (Diff) (%)', 'Value': pf_ratio_clean},
            {'Parameter': 'Clean_DF_Percentage (Diff) (%)', 'Value': df_ratio_clean}
        ])
